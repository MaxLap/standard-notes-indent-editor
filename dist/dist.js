...
function IndentEditor(target_textarea) {
  ...
  function moveSelectedLinesUp(cm) {
    ...
    for (var i = 0; i < lineRanges.length; i++) {
      ...
      cm.replaceRange(cm.doc.lineSeparator() + prevLineContent, {
        line: endLine,
        ch: chOfEndLine
      }, {
        line: endLine,
  ...
  function moveSelectedLinesDown(cm) {
    ...
    for (var i = 0; i < lineRanges.length; i++) {
      ...
      cm.replaceRange('', {
        ...
        line: endLine + 1,
        ch: cm.doc.getLine(endLine + 1).length
      }, "+input");
      cm.replaceRange(nextLineContent + cm.doc.lineSeparator(), {
        line: startLine,
        ch: 0
      }, {
        line: startLine,
      ...
      token: function token(stream, state) {
        ...
        if (toInvestigate) {
          ...
          if (toInvestigate[toInvestigate.length - 1] == '`') {
            ...
            while ((linkToConsider = stackOflinksOnLine[stackOflinksOnLine.length - 1]) && linkToConsider.index < stream.start) {
